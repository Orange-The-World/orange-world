...
//
// It is typed structurally, not as supabase-js's PostgrestFilterBuilder. That
// type is generic over a generated Database schema and this repository has none:
// the exporters name their tables and columns as plain strings. So the shape
// below is exactly the filter surface the exporters use today, and every method
// returns the query so the calls chain. A filter that is not listed here is a
// compile error rather than a silent any, which is the right failure: add the
// one line when a caller needs it.
