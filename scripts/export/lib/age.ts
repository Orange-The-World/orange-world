...
export const CADENCE: Record<string, CadenceRule> = {
  ...
  "us-cpi-monthly.json": {
    ...
    reason:
      ...
      "January): 31 + 31 - 1 + 21 = 82 days, which is what the test in " +
      "age.test.ts asserts. 90 leaves eight days of slack on top of that. " +
      "82 is reached only in those two pairs, but every other pair except " +
      "the two containing February reaches 81, so a threshold of 80 would " +
      "go red in ten months of the twelve, not twice a year. " +
      ...
      "covered month during past federal shutdowns. If a release slips " +
      "past about 35 days a healthy 31 + 31 pair reaches 96 and 90 goes " +
      "red. That red is a source delay, not a pipeline fault, and it is " +
      "not a reason to raise this number.",
  },
